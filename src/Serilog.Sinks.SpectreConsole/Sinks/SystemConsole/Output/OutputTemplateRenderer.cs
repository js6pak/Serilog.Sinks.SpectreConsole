...
{
    class OutputTemplateRenderer : ISpectreFormatter
    {
        ...
        public IEnumerable<IRenderable> Format(LogEvent logEvent)
        {
            if (logEvent is null) throw new ArgumentNullException(nameof(logEvent));

            foreach (var renderer in _renderers)
            {
                foreach (var renderable in renderer.Render(logEvent))
                {
