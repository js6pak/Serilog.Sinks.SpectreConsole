...
namespace Serilog.Sinks.SystemConsole.Output
{
    class MessageTemplateOutputTokenRenderer : OutputTemplateTokenRenderer
    {
        ...
        public override IEnumerable<IRenderable> Render(LogEvent logEvent)
        {
            ...
            {
                return _renderer.Render(logEvent.MessageTemplate, logEvent.Properties);
            }

            return new[] { new RenderableCollection(_renderer.Render(logEvent.MessageTemplate, logEvent.Properties), _token.Alignment) };
        }
    }
}